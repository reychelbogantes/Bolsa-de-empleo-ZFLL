...
import { IS_MOCK, mockRegister } from '../mockAuth.js';

const BASE_URL = import.meta.env.VITE_API_URL ?? 'http://localhost:8000/api';

// ── 1. Registro de Aspirante ──────────────────────────────────
export async function registerAspirante({ nombre, email, password, phone }) {
  if (IS_MOCK) return mockRegister({ email, password, role: 'aspirante', nombre });
...
export async function registerEmpresa({ nombre, email, password, phone, nombre_empresa, cedula_juridica, ubicacion }) {
  if (IS_MOCK) return mockRegister({ email, password, role: 'empresa', nombre: nombre_empresa || nombre });
...
export async function registerInstitucion({ nombre, email, password, phone, nombre_institucion, cedula_juridica, ubicacion }) {
  if (IS_MOCK) return mockRegister({ email, password, role: 'institucion', nombre: nombre_institucion || nombre });
